list_get([L1|_],0,L1).
list_get([_|L],J,R) :- list_get(L,J1,R), J is J1+1.
...
list_blend([],[],_,[]).
list_blend([A1|A], [B1|B], W, [C1|C]) :- list_blend(A,B,W,C), C1 is W*A1+(1-W)*B1.
mat_blend([],[],_,[]).
mat_blend([A1|A], [B1|B], W, [C1|C]) :- list_blend(A1,B1,W,C1), mat_blend(A,B,W,C).
